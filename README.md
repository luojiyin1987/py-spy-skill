...
Analyze the generated flamegraph:
...
For SVG flamegraphs generated by `py-spy record`, use the built-in analyzer first:

```bash
./py-spy-helper.sh analyze-flamegraph profile.svg profile-analysis.md 10
```

The analyzer extracts SVG frame titles, sorts frames by sample percentage, applies coarse bottleneck categories, and emits a Markdown report. Treat that report as a first-pass triage result, not a final performance verdict.
